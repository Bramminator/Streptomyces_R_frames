...
all_runs <- list.dirs("/hosts/linuxhome/mutant7/tmp/bramve/", full.names = TRUE, recursive = FALSE)
# Select runs of interest
runs <- grep('evolving_mutation_100seededwt_r', all_runs, value = TRUE)
# runs <- runs[-1]
...
saveRDS(Population_frame, '~/R/evolving_mutation_100wt_popframe.rds')
...
saveRDS(costs_frame, '~/R/evolving_mutation_100wt_costframe.rds')
